...
void main() {
  ...
  for (final command in ['interpola', 'WELD']) {
    for (final cloud in [false, true]) {
      for (final parameterConfiguration in [
        'complete',
        'without order',
        'without geometry',
      ]) {
        for (final missingCurve in [
          ...
          if (!cloud && missingCurve != null) continue;
          testWidgets(
            '$command (cloud=$cloud, parameters=$parameterConfiguration, invalid assignment: $missingCurve)',
            (tester) async {
                ...
                settings: LaserRobotSettings(
                  ...
                  parametri: [
                    for (final key in [
                      ...
                      if (parameterConfiguration != 'without geometry' &&
                          (parameterConfiguration != 'without order' ||
                              (key != 'path.point_order' &&
                                  key != 'path.perimeter_order')))
              ...
              final free = Point(
                x: 99,
                dashboardPosition: const Offset(10, 20),
              );
              ...
              if (missingCurve == null) {
                ...
                expect(
                  payload['path.perimeter_order'],
                  cloud ? [0, 1, 2, 3] : null,
                );
                expect(
                  payload['path.base_curve_indices'],
                  cloud ? [0, 1] : null,
                );
                expect(
                  payload['path.limit_curve_indices'],
                  cloud ? [2, 3] : null,
                );
              } else {

...
void main() {
  ...
  testWidgets(
    'setup failure consumes output error and permits another attempt',
    (tester) async {
  ...
  testWidgets(
    'async cancellation and destroy errors cannot interrupt cleanup',
    (tester) async {
      ...
      expect(
        f.logs.any((line) => line.contains('[TCP CANCEL')),
        isTrue,
        reason: f.logs.join('\n'),
      );
  ...
  testWidgets(
    'notification errors do not kill connection or command timeouts',
    (tester) async {
      final f = Fixture(
        tester,
  ...
  testWidgets(
    'invalid UTF8 closes just the socket and the next session works',
    (tester) async {
      final f = Fixture(tester);
      await f.connection.connect('robot');
  ...
  test(
    'JSON framing handles every split, concatenation, quotes and escapes',
    () {
      final objects = [
        {
      ...
      expect(
        () => RobotJsonBuffer(maxMessageLength: 8).add('{"large":').toList(),
  ...
  testWidgets('timeout and disconnect produce unknown with no replay', (
    ...
    await tester.pump(const Duration(seconds: 16));
    expect(receipt.outcome.status, RobotCommandStatus.unknown);
    expect(f.connection.socket, same(oldSocket));
    expect(oldSocket.destroyed, isFalse);
  ...
  testWidgets('silence before first message never closes or reconnects TCP', (
    tester,
  ) async {
    final f = Fixture(tester);
    f.connection.startReconnecting('robot');
    await f.connection.connect('robot');
    final old = f.socket;
    final session = f.connection.session;
    await tester.pump(const Duration(minutes: 30));
    await tester.pump(const Duration(seconds: 5));
    expect(old.destroyed, isFalse);
    expect(f.connection.socket, same(old));
    expect(f.connection.session, session);
    expect(f.attempts, 1);
  ...
  testWidgets(
    'remote TCP closure still reconnects without replaying commands',
    (tester) async {
      final f = Fixture(tester);
      f.connection.startReconnecting('robot');
      await f.connection.connect('robot');
      ...
      await tester.pump(const Duration(seconds: 5));
      expect(f.attempts, 2);
      expect(f.connection.socket, isNot(same(old)));
      expect(f.connection.state, RobotConnectionState.awaitingData);
      expect(f.socket.writes, isEmpty);
      f.connection.dispose();
    },
  ...
  testWidgets('missing RobotStatus with other traffic keeps the same session', (
    tester,
  ) async {
    final f = Fixture(tester);
    f.connection.startReconnecting('robot');
    ...
    expect(old.destroyed, isFalse);
    expect(f.connection.session, session);
    expect(f.connection.state, RobotConnectionState.online);
    expect(f.attempts, 1);
    expect(f.received, hasLength(8));
    expect(old.writes, isEmpty);
    f.connection.dispose();
  });

  testWidgets(
    'telemetry resumes after prolonged silence without reconnecting',
    (tester) async {
      final f = Fixture(tester);
      f.connection.startReconnecting('robot');
      await f.connection.connect('robot');
      ...
      final command = f.connection.send({'f': 'getPoint'});
      await tester.pump(const Duration(minutes: 30));
      await tester.pump(const Duration(seconds: 5));
      expect(command.outcome.status, RobotCommandStatus.unknown);
      expect(f.connection.socket, same(old));
      expect(old.destroyed, isFalse);
      expect(f.connection.session, session);
      expect(f.attempts, 1);
      expect(old.writes, hasLength(1));
      old.message({'f': 'RobotStatus', 'status': 'PAUSED'});
      await tester.pump();
      expect(f.received.last['MSG']['status'], 'PAUSED');
      expect(f.connection.state, RobotConnectionState.online);
      expect(f.connection.send({'f': 'STOPCORDONE'}).accepted, isTrue);
      expect(old.writes, hasLength(2));
      f.connection.dispose();
    },
  );
  ...
  test(
    'real localhost TCP preserves outgoing JSON and incoming UTF8',
    () async {
      ...
      try {
        ...
        expect(receipt.outcome.status, RobotCommandStatus.waiting);
        final bytes = utf8.encode(
          '{"MSG":{"f":"RobotInfo","Model":"modalità"}}',
        );
        final split = bytes.indexOf(0xc3) + 1;

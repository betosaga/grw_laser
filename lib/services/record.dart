...
class WellknownUriRecord implements Record {
  ...
  static WellknownUriRecord fromNdef(NdefRecord record) {
    final prefix = _URI_PREFIX_LIST[record.payload.first];
  ...
  NdefRecord toNdef() {
    var prefixIndex =
        _URI_PREFIX_LIST.indexWhere((e) => uri.toString().startsWith(e), 1);
    if (prefixIndex < 0) prefixIndex = 0;
    final prefix = _URI_PREFIX_LIST[prefixIndex];
    return NdefRecord(
      typeNameFormat: TypeNameFormat.wellKnown,
...
const List<String> _URI_PREFIX_LIST = [
  "",
  "http://www.",
  "https://www.",
  "http://",
